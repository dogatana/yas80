...
const (
	...
	ECHARMAP_DEFCHAR_VALUE = "デフォルトコードが誤り"
	// 適用
	...
	EZ80_RST              = "RST のアドレスは 0/8/10H/18H/20H/28H/30H/38H のいずれか(指定値: %d(0x%x))"
	EZ80_JR_RANGE         = "相対ジャンプ先が範囲外 %d(0x%x)"
	EZ80_JR_FLAG          = "条件付き JR で %s フラグは使用不可"
	...
	EVAR_VALUE        = "変数 %s の初期値誤り"
	EVAR_VALUE_NULL   = "変数 %s の初期値が未確定"
	EVAR_VALUE_FWD    = "変数 %s の初期値は前方参照不可"
	...
	EENUM_ELE_NULL  = "ENUM 要素の値が未確定"
	EENUM_ELE_FWD   = "ENUM 要素の値は前方参照不可"
	// ENUM 参照
