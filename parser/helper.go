...
func parseInt(s string) (int64, error) {
	...
	case length >= 2 && str[0] == '0' && (str[1] == 'X' || str[1] == 'O' || str[1] == 'B'):
		// 0x 0o 0b
		return strconv.ParseInt(str, 0, 0)
	case length >= 2 && (str[length-1] == 'H'):
...
func buildInfixExpression(opcode int, op1, op2 Expression, ctx *filecontent.Context) Expression {
...
func PrintNode(stmt Statement, indent int) {
	...
	case *MacroBlockStatement:
		fmt.Printf("%s[MacroBlockStataement %s %d]\n", strings.Repeat(" ", indent*2), stmt.NameID, stmt.Start)
		for _, s := range stmt.Block {
			PrintNode(s, indent+1)
		}
	case *IfStatement:
