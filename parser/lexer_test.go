...
func TestLexBlankInput(t *testing.T) {
	...
	for _, tt := range tests {
		...
		for _, expected := range tt.expected_tokens {
			tok := l.NextToken()
			if tt.input != "" && tok.Position.LineNumber == 0 {
...
func TestLexerLineNumber(t *testing.T) {
	...
	for _, tt := range tests {
		...
		for {
			...
			if tok.Position.LineNumber != ln {
				t.Errorf("LineNumber not %d. got %d", ln, tok.Position.LineNumber)
			}
