...
func (e *Evaluator) ebMacroEntry(stmt *parser.MacroCallStatement, env TEnv) object.Object {
	...
	if len(args) != 1 {
		e.logger.Error(fmt.Sprintf(errcode.EEBMAC_ARG_COUNT, stmt.NameID), stmt.Context)
		return object.ERROR
	}

	var v int
	obj := e.evalMacroNumberArg(stmt.NameID, args[0], env, stmt.Context)
	if no, ok := obj.(*object.NumberObject); !ok {
		return obj
	} else {
		v = no.Value
	}
...
func (e *Evaluator) ebMacroAlign(stmt *parser.MacroCallStatement, env TEnv) object.Object {
	...
	if len(args) == 2 {
		// あれば第2オペランド
		obj = e.evalMacroNumberArg(stmt.NameID, args[1], env, stmt.Context)
	} else {
		// なければ $FILL
		obj, _ = env.Get(intern.ID_FILL)
	}
	...
	b, ok := e.intToByte(no.Value)
	if !ok {
		e.logger.Warning(fmt.Sprintf(errcode.WROUND_BYTE, no.Value, no.Value), stmt.Context)
	}
	...
	// Z80Instruction でないため、直接アドレスを進める
...
func (e *Evaluator) ebMacroLogMessage(msgType int, stmt *parser.MacroCallStatement, env TEnv) object.Object {
	...
	case *object.NullObject:
		e.logger.Error(fmt.Sprintf(errcode.EEBMAC_ARG_NULL, stmt.NameID), stmt.Context)
		return object.ERROR
	default:
		e.logger.Error(fmt.Sprintf(errcode.EEBMAC_ARG_VALUE, stmt.NameID), stmt.Context)
		return object.ERROR
	}
...
func (e *Evaluator) ebMacroIncBin(stmt *parser.MacroCallStatement, env TEnv) object.Object {
	// $RSIZE 初期化
	id := intern.InternString("$RSIZE")
	if obj, ok := env.Get(id); !ok {
		env.Set(id, &object.NumberObject{Value: -1})
	} else {
	...
	obj := e.evalMacroStringArg(stmt.NameID, args[0], env, stmt.Context)
	...
	size := 0 // 読み込みサイズ
	if len(args) > 2 {
		obj := e.evalMacroNumberArg(stmt.NameID, args[2], env, stmt.Context)
		if obj, ok := obj.(*object.NumberObject); !ok {
			return obj
		} else {
			size = obj.Value
...
func (e *Evaluator) ebMacroSetMap(stmt *parser.MacroCallStatement, env TEnv) object.Object {
	...
	var str string
	obj = e.evalMacroStringArg(stmt.NameID, args[1], env, stmt.Context)
	if obj, ok := obj.(*object.StringObject); !ok {
		return obj
	} else {
		str = obj.Value
	...
	case *object.ArrayObject:
		...
		for i, v := range obj.Values {
			...
			b, ok := e.intToByte(no.Value)
			if !ok {
				e.logger.Warning(fmt.Sprintf(errcode.WROUND_BYTE, v, v), stmt.Context)
			}
			values[i] = b
		}
	...
	return &object.CodeTextObject{Text: fmt.Sprintf("%q:%s", str, atext), Context: stmt.Context}
}
...
func (e *Evaluator) evalMacroStringArg(id intern.SymbolID, expr parser.Expression, env TEnv, ctx TContext) object.Object {
	...
	case *object.RefNotFoundObject:
		return obj
	case *object.NullObject:
		e.logger.Error(fmt.Sprintf(errcode.EEBMAC_ARG_NULL, id), ctx)
		return object.ERROR
	...
	default:
		e.logger.Error(fmt.Sprintf(errcode.EEBMAC_ARG_VALUE, id), ctx)
		return object.ERROR
	}
}

// マクロ引数を NumberObject として評価
func (e *Evaluator) evalMacroNumberArg(id intern.SymbolID, expr parser.Expression, env TEnv, ctx TContext) object.Object {
	obj := e.evalExpression(expr, env, ctx)
	switch obj := obj.(type) {
	case *object.ErrorObject:
		return obj
