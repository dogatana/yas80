...
func TestBuiltinFuncLength(t *testing.T) {
	tests := []struct {
		...
		{input: `const v = $len(xyz)`, err: errcode.ESYM_UNDEF},
	}
...
func TestBuiltinFuncIsArray(t *testing.T) {
	tests := []struct {
		...
		{input: `const v = $isary(xyz)`, err: errcode.ESYM_UNDEF},
	}
...
func TestBuiltinFuncReverse(t *testing.T) {
	tests := []struct {
		...
		{input: `const abc = $rev(def)`, err: errcode.ESYM_UNDEF},
	}
	for tn, tt := range tests {
		if tt.input == "" {
			continue
...
func TestBuiltinFuncWord(t *testing.T) {
	tests := []struct {
		...
		{input: `fn func \ endf \ db $w(fn())`, err: errcode.EEBFN_ARG_NULL},
	}
	for tn, tt := range tests {
		if tt.input == "" {
			continue
...
func TestBuiltinFuncDefined(t *testing.T) {
	tests := []struct {
		input string
		code  []byte
		syms  []symValue
		...
		// 0-
		{input: `const v = $defined(x)`, syms: []symValue{{"V", 0}}},
		{input: `const v = $defined(v)`, syms: []symValue{{"V", 1}}},
		{input: `const x = 1 \ const v = $defined(x)`, syms: []symValue{{"V", 1}}},
	}
	for tn, tt := range tests {
		if tt.input == "" {
			continue
