...
func (e *Evaluator) expandMacro(mcall *parser.MacroCallStatement, macro *object.MacroObject, checkExitM bool, ectx TContext, env TEnv) object.Object {
	...
	mfn := buildMangleNamesFunc(args, seq, name)
	...
	// ENDM コメント追加
	cs := &parser.CommentStatement{Text: fmt.Sprintf("endm(%s)", name), Context: ectx}
	stmts = append(stmts, cs)

	// トップレベルの mbc は Context 置き換えしない
	mbc := &parser.MacroBlockStatement{Label: mcall.Label, NameID: mcall.NameID, Block: stmts, Context: mcall.Context}
	for _, s := range stmts {
...
func buildMangleNamesFunc(args map[string]parser.Expression, seq int, macroName string) func(ptr *parser.Expression) {
	...
	fn = func(ptr *parser.Expression) {
		...
		case *parser.Label:
			...
			if name[0] == '@' {
				newLabel := *expr
				newLabel.NameID = intern.InternString(mangleName(seq, macroName, name))
				newLabel.LabelType = parser.NODE_LABEL
				*ptr = &newLabel
			} else if arg, ok := args[name]; ok {
				*ptr = arg
			}
		...
		case *parser.Ident:
			...
			if name[0] == '@' {
				newIdent := *expr
				newIdent.NameID = intern.InternString(mangleName(seq, macroName, name))
