...
func (e *Evaluator) evalZ80Instruction(node *parser.Z80Instruction, env object.Environment) object.Object {
	...
	case parser.Z80_INST0:
		...
		obj := &object.CodeObject{Line: node.TokenContext.LineNumber, Code: make([]byte, len(info.Bytes))}
...
func (e *Evaluator) generateRET(node *parser.Z80Instruction, _ object.Environment) object.Object {
	if node.Op1 == nil {
		return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{0xc9}}
	}
	if node.Op1.NodeType() == parser.Z80_FLAG {
		flag := int(node.Op1.NodeSubType()) - parser.Z80_FLAG_NZ
		b := byte(0xc0 | flag<<3)
		return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{b}}
	}
	e.logger.Error(
		fmt.Sprintf(errcode.E017, node.Op1.String()), node.TokenContext.LineNumber)
	return object.ERROR
}
...
func (e *Evaluator) evalZ80Instruction2(node *parser.Z80Instruction, env object.Environment) object.Object {
	...
	default:
		e.logger.Error(fmt.Sprintf(errcode.E999, node), node.TokenContext.LineNumber)
		return object.ERROR
	}
}
...
func (e *Evaluator) evalZ80LD(node *parser.Z80Instruction, env object.Environment) object.Object {
	...
	case *object.RegisterObject:
		...
		e.logger.Error(errcode.E024, node.TokenContext.LineNumber)
		return object.ERROR
	...
	default:
		if e.Pass1 {
			...
			return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{0x7f}}
		}
		e.logger.Error(errcode.E024, node.TokenContext.LineNumber)
		return object.ERROR
	}
...
func (e *Evaluator) evalZ80LD_reg8(node *parser.Z80Instruction, op1 *object.RegisterObject, env object.Environment) object.Object {
	...
	case *object.RegisterObject:
		...
		if op2.RegisterType != parser.Z80_REG8 {
			e.logger.Error(errcode.E025, node.TokenContext.LineNumber)
			return object.ERROR
		}
		r1, ok := Z80Reg8Index[int(op1.Register)]
		if !ok {
			e.logger.Error(fmt.Sprintf(errcode.E028, parser.TokenLiteral(op1.Register)), node.TokenContext.LineNumber)
			return object.ERROR
		}
		r2, ok := Z80Reg8Index[int(op2.Register)]
		if !ok {
			e.logger.Error(fmt.Sprintf(errcode.E028, parser.TokenLiteral(op2.Register)), node.TokenContext.LineNumber)
			return object.ERROR
		}

		b := 0x40 | r1<<3 | r2
		return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{byte(b)}}
	case *object.NumberObject:
		// LD r, n
		v, ok := e.intToByte(op2.Value)
		if !ok {
			e.logger.Warning(fmt.Sprintf(errcode.W001, op2.Value, op2.Value), node.TokenContext.LineNumber)
		}
		r1 := Z80Reg8Index[int(op1.Register)]
		b := byte(0x06 | (r1 << 3))
		return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{b, v}}
	// case *object.IndirectExpression:
	// 	e.logger.Error(fmt.Sprintf(errcode.E999, node), node.TokenContext.LineNumber)
	// 	return object.ERROR
...
func (e *Evaluator) evalZ80LD_reg16(node *parser.Z80Instruction, op1 *object.RegisterObject, env object.Environment) object.Object {
	...
	case *object.RegisterObject:
		...
		if op1.Register != parser.Z80_REG_SP {
			e.logger.Error(errcode.E026, node.TokenContext.LineNumber)
			return object.ERROR
		}
		...
		case parser.Z80_REG_HL:
			return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{0xf9}}
		case parser.Z80_REG_IX:
			return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{0xdd, 0xf9}}
		case parser.Z80_REG_IY:
			return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{0xfd, 0xf9}}
		default:
			e.logger.Error(errcode.E027, node.TokenContext.LineNumber)
			return object.ERROR
		}
	case *object.NumberObject:
		// LD rr, nn
		...
		if !ok {
			e.logger.Warning(fmt.Sprintf(errcode.W002, op2.Value, op2.Value), node.TokenContext.LineNumber)
		}
		r1 := Z80Reg16Index[int(op1.Register)]
		b := byte(0x01 | (r1 << 4))
		return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{b, byte(v & 0xff), byte((v >> 8) & 0xff)}}
	// case *object.IndirectExpression:
	// 	e.logger.Error(fmt.Sprintf(errcode.E999, node), node.TokenContext.LineNumber)
	// 	return object.ERROR
	case *object.SymbolObject:
		...
		if ok {
			...
			if !ok {
				e.logger.Warning(fmt.Sprintf(errcode.W002, op2v.Value, op2v.Value), node.TokenContext.LineNumber)
			}
			r1 := Z80Reg16Index[int(op1.Register)]
			b := byte(0x01 | (r1 << 4))
			return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{b, byte(v & 0xff), byte((v >> 8) & 0xff)}}
		...
		if e.Pass1 {
			// pass1 の場合はダミーとして LD A, A を返す
			return &object.CodeObject{Line: node.TokenContext.LineNumber, Code: []byte{0x21, 0, 0}}
		}
		e.logger.Error(errcode.E025, node.TokenContext.LineNumber)
		return object.ERROR
	default:
