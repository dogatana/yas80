...
func (e *Evaluator) evalDataStoreStatement(stmt *parser.DataStoreStatement, env TEnv) object.Object {
	...
	case *object.RefNotFoundObject:
		count = 1 // 未確定なら 1 として処理を続行
	case *object.NullObject:
		e.logger.Error(errcode.EDS_COUNT_NULL, stmt.Context)
		return object.ERROR
	...
	default:
		e.logger.Error(errcode.EDS_COUNT_VALUE, stmt.Context)
		return object.ERROR
	}
	...
	if stmt.FillValue == nil {
		...
		case *object.RefNotFoundObject:
			// 未確定なら $FILL 値を利用
		case *object.NullObject:
			e.logger.Error(errcode.EDS_FILL_NULL, stmt.Context)
			return object.ERROR
		...
		default:
			e.logger.Error(errcode.EDS_FILL_VALUE, stmt.Context)
			return object.ERROR
		}
	}
