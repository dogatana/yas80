...
func (e *Evaluator) Eval(node parser.Node, env object.Environment) object.Object {
	...
	case *parser.EnumStatement:
		...
		if ok {
			e.logger.Error(fmt.Sprintf(errcode.E012, name), node.Context.Line)
			return object.ERROR
		}
	...
	case *parser.Ident:
		...
		if !ok && e.Pass1 {
			...
			e.logger.Error(fmt.Sprintf(errcode.E009, uname), node.Context.Line)
			return object.ERROR
		}
	...
	case *parser.DotIdent:
		...
		if !ok {
			e.logger.Error(fmt.Sprintf(errcode.E010, node.Left), node.Context.Line)
			return object.ERROR
		}
		v, ok := enum.(*object.EnumObject).Get(node.Right)
		if !ok {
			e.logger.Error(fmt.Sprintf(errcode.E011, node.Left, node.Right), node.Context.Line)
			return object.ERROR
		}
		return v
...
func (e *Evaluator) evalMacroStatement(node *parser.MacroStatement, env object.Environment) object.Object {
	if _, ok := env.Get(node.Name); ok {
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_DEF, node.Name), node.Context.Line)
		return object.ERROR
	}
...
func (e *Evaluator) evalMacroCallStatement(node *parser.MacroCallStatement, env object.Environment) object.Object {
	...
	if !ok && len(node.Args.Expressions) == 1 {
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_FUNC_NOT_FOUND, node.Name), node.Context.Line)
		return object.ERROR
	} else if !ok {
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_NOT_FOUND, node.Name), node.Context.Line)
		return object.ERROR
	}
	...
	default:
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_NOT_MACRO, node.Name), node.Context.Line)
		return object.ERROR
	}
}
...
func (e *Evaluator) evalMacroBody(node *parser.MacroCallStatement, macro *object.MacroObject, env object.Environment) object.Object {
	...
	if len(node.Args.Expressions) != len(macro.Params) {
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_ARG_COUNT, macro.Name), node.Context.Line)
		return object.ERROR
	}
	...
	if obj, ok := env.Get(name); ok {
		...
		case *object.SymbolObject:
			if obj.SymType != object.LABEL || obj.LineNumber != node.Context.Line || obj.SymState == object.VALUE_DETERMINED {
				if !e.Pass1 {
					e.logger.Error(fmt.Sprintf(errcode.E032, name), node.Context.Line)
				}
				return object.ERROR
			}
			// fall through
...
func (e *Evaluator) evalConstStatement(node *parser.ConstStatement, env object.Environment) object.Object {
	...
	if _, ok := env.Get(name); ok {
		e.logger.Error(fmt.Sprintf(errcode.E031, name), node.Context.Line)
		return object.ERROR
	}
	...
	case *object.RefNotFoundObject:
		...
		if !e.Pass1 {
			e.logger.Error(fmt.Sprintf(errcode.E009, strings.Join(v.Names, ", ")), node.Context.Line)
			return object.ERROR
		}
		// 未定義定数として登録
...
func (e *Evaluator) evalFuncStatement(stmt *parser.FuncStatement, env object.Environment) object.Object {
	...
	_, ok := env.Get(name)
	if ok {
		e.logger.Error(fmt.Sprintf(errcode.E018, name), stmt.Context.Line)
		return object.NULL
	}
...
func (e *Evaluator) evalEnumStatement(node *parser.EnumStatement, env object.Environment) object.Object {
	...
	for _, ele := range node.Elements.Elements {
		...
		if _, ok := enum[eleName]; ok {
			e.logger.Error(fmt.Sprintf(errcode.E013, node.Name, ele.Name), node.Context.Line)
			return object.ERROR
		}
