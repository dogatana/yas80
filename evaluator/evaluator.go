...
func (e *Evaluator) Eval(node parser.Node, env object.Environment) object.Object {
	...
	case *parser.Z80Instruction:
		e.lineNumber = node.Position.LineNumber
	...
	case *parser.EnumStatement:
		...
		if ok {
			e.logger.Error(fmt.Sprintf(errcode.E012, name), node.Position.LineNumber)
			return object.ERROR
		}
	...
	case *parser.NumberLiteral:
		return &object.NumberObject{Value: node.Value, LineNumber: node.Position.LineNumber}
	case *parser.StringLiteral:
		return &object.StringObject{Value: node.Value, LineNumber: node.Position.LineNumber}
	case *parser.FlagLiteral:
		return &object.StringObject{Value: node.String(), LineNumber: node.Position.LineNumber}
	case *parser.InfixExpression:
		return e.evalInfixExpression(node, env, node.Position.LineNumber)
	case *parser.PrefixExpression:
		v := e.Eval(node.Op, env)
		if isError(v) || isRefNotFound(v) {
			return v
		}
		return e.evalPrefixExpression(node.Operator, v, node.Position.LineNumber)
	case *parser.Ident:
		uname := node.Name
		...
		if !ok && e.Pass1 {
			...
			e.logger.Error(fmt.Sprintf(errcode.E009, uname), node.Position.LineNumber)
			return object.ERROR
		}
	...
	case *parser.DotIdent:
		...
		if !ok {
			e.logger.Error(fmt.Sprintf(errcode.E010, node.Left), node.Position.LineNumber)
			return object.ERROR
		}
		v, ok := enum.(*object.EnumObject).Get(node.Right)
		if !ok {
			e.logger.Error(fmt.Sprintf(errcode.E011, node.Left, node.Right), node.Position.LineNumber)
			return object.ERROR
		}
		return v
...
func (e *Evaluator) evalMacroStatement(node *parser.MacroStatement, env object.Environment) object.Object {
	if _, ok := env.Get(node.Name); ok {
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_DEF, node.Name), node.Position.LineNumber)
		return object.ERROR
	}
...
func (e *Evaluator) evalMacroCallStatement(node *parser.MacroCallStatement, env object.Environment) object.Object {
	...
	if !ok && len(node.Args.Expressions) == 1 {
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_FUNC_NOT_FOUND, node.Name), node.Position.LineNumber)
		return object.ERROR
	} else if !ok {
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_NOT_FOUND, node.Name), node.Position.LineNumber)
		return object.ERROR
	}
	...
	default:
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_NOT_MACRO, node.Name), node.Position.LineNumber)
		return object.ERROR
	}
}
...
func (e *Evaluator) evalMacroBody(node *parser.MacroCallStatement, macro *object.MacroObject, env object.Environment) object.Object {
	...
	if len(node.Args.Expressions) != len(macro.Params) {
		e.logger.Error(fmt.Sprintf(errcode.EMACRO_ARG_COUNT, macro.Name), node.Position.LineNumber)
		return object.ERROR
	}
...
func (e *Evaluator) evalLabelStatement(node *parser.LabelStatement, env object.Environment) object.Object {
	...
	if obj, ok := env.Get(name); ok {
		switch obj := obj.(type) {
		case *object.SymbolObject:
			if obj.SymType != object.LABEL || obj.LineNumber != node.Position.LineNumber || obj.SymState == object.VALUE_DETERMINED {
				if !e.Pass1 {
					e.logger.Error(fmt.Sprintf(errcode.E032, name), node.Position.LineNumber)
				}
				return object.ERROR
			}
			// fall through
	...
	sym := object.NewLabelSymbol(name, getLocationCounter(env), node.Position.LineNumber)
	env.Set(name, sym)
	return sym
}
...
func (e *Evaluator) evalConstStatement(node *parser.ConstStatement, env object.Environment) object.Object {
	...
	if _, ok := env.Get(name); ok {
		e.logger.Error(fmt.Sprintf(errcode.E031, name), node.Position.LineNumber)
		return object.ERROR
	}
	...
	case *object.RefNotFoundObject:
		...
		if !e.Pass1 {
			e.logger.Error(fmt.Sprintf(errcode.E009, strings.Join(v.Names, ", ")), node.Position.LineNumber)
			return object.ERROR
		}
		// 未定義定数として登録
		sym := object.NewNullConstSymbol(name, node.Value, v.Names, node.Position.LineNumber)
		env.Set(name, sym)
		return sym
	case *object.SymbolObject:
		...
		sym := object.NewNullConstSymbol(name, node.Value, depends, node.Position.LineNumber)
		env.Set(name, sym)
		return sym
...
func (e *Evaluator) evalIfStatement(stmt *parser.IfStatement, env object.Environment) object.Object {
	...
	if !ok {
		return &object.NodeObject{Value: stmt, LineNumber: stmt.Position.LineNumber}
	}
	if cond.Value != 0 {
...
func (e *Evaluator) evalFuncStatement(stmt *parser.FuncStatement, env object.Environment) object.Object {
	...
	_, ok := env.Get(name)
	if ok {
		e.logger.Error(fmt.Sprintf(errcode.E018, name), stmt.Position.LineNumber)
		return object.NULL
	}
...
func (e *Evaluator) evalReturnStatement(stmt *parser.ReturnStatement, env object.Environment) object.Object {
	...
	return &object.ReturnObject{Value: ret, LineNumber: stmt.Position.LineNumber}
}
...
func (e *Evaluator) evalEnumStatement(node *parser.EnumStatement, env object.Environment) object.Object {
	...
	for _, ele := range node.Elements.Elements {
		...
		if _, ok := enum[eleName]; ok {
			e.logger.Error(fmt.Sprintf(errcode.E013, node.Name, ele.Name), node.Position.LineNumber)
			return object.ERROR
		}
